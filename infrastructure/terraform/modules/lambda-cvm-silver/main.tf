...
resource "aws_iam_policy" "lambda_execution" {
  ...
  description = "Least-privilege permissions for the CVM RAW to Silver Lambda."
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "${var.data_lake_bucket_arn}/raw/cvm/*"
      },
      {
        Sid    = "ReadWriteCVMSilver"
        Effect = "Allow"
        ...
        Resource = "${var.data_lake_bucket_arn}/silver/cvm/*"
      },
      {
...
resource "aws_lambda_function" "cvm_raw_to_silver" {
